	...
	namespace overlays
	{
			...
			: home_menu_page(x, y, width, height, use_separators, parent, get_localized_string(localized_string_id::HOME_MENU_SAVESTATE))
		{
			std::unique_ptr<overlay_element> save_state = std::make_unique<home_menu_entry>(
				get_localized_string(localized_string_id::HOME_MENU_SAVESTATE_SAVE));

			add_item(save_state, [](pad_button btn) -> page_navigation
				{
			...
			add_item(reload_state, [this](pad_button btn) -> page_navigation
				{
					if (btn != pad_button::cross)
						return page_navigation::stay;
					...
					if (!boot_last_savestate(true))
					{
						show_dialog(get_localized_string(localized_string_id::HOME_MENU_NO_SAVESTATE));
						return page_navigation::stay;
					}
