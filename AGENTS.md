...
- Imported slice 2026-05-12: Vulkan shader interpreter now uses the existing deferred pipeline compiler for preload work, avoids duplicating interpreter uniform tables per variant, prebuilds common interpreter shader-module variants, preloads representative interpreter pipelines when `async_with_interpreter` or `interpreter_only` is active, and logs interpreter cache misses over 1000 ms with the compiler option mask.
