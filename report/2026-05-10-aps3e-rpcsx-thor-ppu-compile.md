...
- `app/src/main/cpp/rpcsx` is now a plain-file vendor copy of the upstream RPCSX core source. The full upstream Android core build entry is `app/src/main/cpp/rpcsx/android/CMakeLists.txt`; the regular Gradle app build still points at the lightweight wrapper CMake file, with source-core packaging gated behind `RPCSX_BUILD_BUNDLED_CORE=1` / `-PbuildBundledRpcsxCore=true`.
- Source-core packaging checkpoint: the wrapper/core library names are separated (`librpcsx-ui-jni.so` for Java JNI, `librpcsx-android.so` for the bundled core), the core Android CMake can be included as a subdirectory, and the upstream third-party dependency trees are pinned as root repo submodules. After `tools/hydrate_rpcsx_core_deps.ps1`, `.\gradlew.bat ':app:configureCMakeDebug[arm64-v8a]' -PbuildBundledRpcsxCore=true` and `.\gradlew.bat :app:assembleDebug -PbuildBundledRpcsxCore=true` both succeed on 2026-05-10. The source-core debug APK was about 162.8 MB and packaged `lib/arm64-v8a/librpcsx-android.so` at about 154 MB.
